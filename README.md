...
[![Build Status](https://travis-ci.org/oltarasenko/the_crawler.svg?branch=master)](https://travis-ci.org/github/oltarasenko/the_crawler)
[![Coverage Status](https://coveralls.io/repos/github/oltarasenko/the_crawler/badge.svg?branch=master)](https://coveralls.io/github/oltarasenko/the_crawler?branch=master)
...
and finally maintenance. And that's what we're trying to achieve with TheCrawler
project.
...
3. `docker-compose run ui bash -c "/crawlyui/bin/ec eval \"TheCrawler.ReleaseTasks.migrate\""`
...
TheCrawler is a phoenix application, which is responsible for working with Crawly
nodes. All nodes are connected to TheCrawler using the erlang distribution. TheCrawler
...
2. Organize erlang cluster so Crawly nodes can find TheCrawler node, in this case we use
...
(which should be same with your TheCrawler session), you can also define a node name
...
### Start TheCrawler session

Start an iex session in TheCrawler directory with `--name <crawlyui-node-name>@127.0.0.1`
...
session ping the TheCrawler node with `Node.ping/1`, example:
