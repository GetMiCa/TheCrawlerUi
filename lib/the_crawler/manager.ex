defmodule TheCrawler.Manager do
  @moduledoc """
  The Manager context.
  """
  ...
  alias TheCrawler.Manager.Job
  alias TheCrawler.Manager.Item
  alias TheCrawler.Manager.Spider
  ...
  def parse_search_string(search_string) do
    case TheCrawler.QueryParser.query(search_string) do
  ...
  def update_spider(name, attrs) do
    ...
    case TheCrawler.Manager.get_spider!(name) do
      nil ->
        {:error, :not_found}

      data ->
        TheCrawler.Manager.Spider.changeset(data, attrs) |> Repo.update()
    end
  end
