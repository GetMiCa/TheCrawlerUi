defmodule TheCrawlerWeb.ItemLive do
  use Phoenix.LiveView

  alias TheCrawler.Manager

  def render(%{template: template} = assigns) do
    TheCrawlerWeb.ItemView.render(template, assigns)
  end
  ...
  def handle_event("show_item", %{"job" => job_id, "item" => item_id}, socket) do
    {:noreply,
     push_redirect(socket,
       to: TheCrawlerWeb.Router.Helpers.item_path(socket, :show, job_id, item_id)
     )}
  end

  def handle_event("job_items", %{"job" => job_id}, socket) do
    {:noreply,
     push_redirect(socket, to: TheCrawlerWeb.Router.Helpers.item_path(socket, :index, job_id))}
  end
  ...
  def handle_event("goto_page", %{"page" => page}, socket) do
    ...
    {:noreply,
     push_redirect(socket,
       to:
         TheCrawlerWeb.Router.Helpers.item_path(socket, :index, job_id,
           page: page,
           search: socket.assigns.search
         )
     )}
  ...
  defp update_interval() do
    settings = Application.get_env(:the_crawler, __MODULE__, [])
