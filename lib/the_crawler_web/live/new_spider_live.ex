defmodule TheCrawlerWeb.NewSpiderLive do
  use Phoenix.LiveView

  def render(assigns) do
    TheCrawlerWeb.JobView.render("new_spider.html", assigns)
  end
  ...
  def mount(%{"spider" => spider}, _session, socket) do
    ...
    case TheCrawler.Manager.get_spider!(spider) do
      nil ->
  ...
  def handle_event("save_spider", _data, socket) do
    ...
    case socket.assigns.updating do
      false ->
        case TheCrawler.Manager.create_spider(attrs) do
          {:ok, _spider} ->
      ...
      true ->
        case TheCrawler.Manager.update_spider(Map.get(socket.assigns.data, "name"), attrs) do
          {:error, reason} ->
