defmodule TheCrawlerWeb.SpiderLive do
  use Phoenix.LiveView, layout: {TheCrawlerWeb.LayoutView, "live.html"}
  ...
  def render(assigns) do
    TheCrawlerWeb.JobView.render("spider.html", assigns)
  end
  ...
  def handle_event("goto_page", %{"page" => page}, socket) do
    ...
    {:noreply,
     push_redirect(socket,
       to: TheCrawlerWeb.Router.Helpers.spider_path(socket, :spider, page: page, spider: spider)
     )}
  end
  ...
  def handle_event("show_spider", %{"spider" => spider}, socket) do
    {:noreply,
     push_redirect(socket,
       to: TheCrawlerWeb.Router.Helpers.spider_path(socket, :spider, spider: spider)
     )}
  end
  ...
  def handle_event("schedule_autospider", %{"node" => node_name, "spider" => spider_name}, socket) do
    TheCrawler.create_spider(node_name, spider_name)

    case TheCrawler.SpiderManager.start_spider(node_name, spider_name) do
      {:ok, :started} ->
        socket =
          socket
    ...
    case SpiderManager.start_spider(node, spider) do
      ...
      error ->
        {:noreply,
         ...
         |> redirect(to: TheCrawlerWeb.Router.Helpers.spider_path(socket, :spider))}
    end
  end
