defmodule TheCrawlerWeb.ItemLiveTest do
  ...
  test "mount view for items index", %{conn: conn, job_id: job_id} do
    ...
    assert TheCrawlerWeb.ItemLive = view.module
  ...
  test "Update item view if job is in running state", %{
    ...
    item = TheCrawler.Manager.get_item!(item_id)
  ...
  test "redirect go to page", %{
    ...
    # page 1
    item_1 = TheCrawler.Manager.get_item!(item_id)
    Enum.each(1..9, &insert_item(job_id, inserted_at(&1)))

    # page 2
    item_2 = insert_item(job_id)

    Application.put_env(:the_crawler, :page_size, 1)
    ...
    Application.put_env(:the_crawler, :page_size, 10)
  end
end
